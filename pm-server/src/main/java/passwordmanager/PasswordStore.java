...
public class PasswordStore {
    ...
    public void storePassword(PublicKey pubKey, byte[] domainUsernameHash, byte[] password, byte[] signature, long timestamp, String uuid)throws UserNotRegisteredException{
        UserData userData = _data.get(Cryptography.encodeForStorage(pubKey.getEncoded()));
        if(null == userData){
            throw new UserNotRegisteredException("User with this public key is not yet registered");
        }

        userData.storePassword(domainUsernameHash, password, signature, timestamp, uuid);
    }
